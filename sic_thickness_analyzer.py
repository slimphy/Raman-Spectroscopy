    ...
    def run(self):
        ...
        for i, ((x, y), indices) in enumerate(grouped_indices.items()):
            ...
            try:
                ...
                # 🚨 [신규] 4. 계면 + 10µm (기판 피크) 추출 및 차이 연산
                target_z_10um = z_intf + 10.0

                # 측정된 최대 Z 깊이가 z_intf + 10 보다 짧을 경우, 가장 마지막 측정값을 대신 사용
                if target_z_10um > np.max(z):
                    idx_10um = -1
                else:
                    idx_10um = np.argmin(np.abs(z - target_z_10um))

                actual_z_10um = z[idx_10um]
                peak_10um_val = peak_weighted_com[idx_10um]

                # 차이 연산 (Surface Peak - Substrate Peak)
                peak_diff = surf_peak_val - peak_10um_val

            except Exception as e:
                thickness, z_surf, z_intf = 0, 0, 0
                surf_peak_val, peak_10um_val, peak_diff, actual_z_10um = np.nan, np.nan, np.nan, np.nan
                popt_i = None

            # 시각화 및 CSV 저장을 위해 결과 통합 보관
            thickness_map[(x, y)] = {
                't': thickness, 'zs': z_surf, 'zi': z_intf, 'z': z,
                'area': area_776, 'peak': peak_weighted_com, 'popt_i': popt_i, 'norm_area': norm_area,
                'surf_peak': surf_peak_val, 'z_10um': actual_z_10um, 'peak_10um': peak_10um_val, 'peak_diff': peak_diff
            }
...
class MapViewer(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("SiC Epi Data Analyzer (Multi-Map View)")
        ...
        self.combo_map_mode.addItems([
            ...
            "🗺️ 모드: 계면+10µm 피크 (Substrate Peak)",
            "🗺️ 모드: 피크 위치 차이 (Surface - Substrate)"
        ])
        ...
        self.txt_info = pg.TextItem("", color='w', anchor=(0, 0), fill=pg.mkBrush(0, 0, 0, 150))
    ...
    def refresh_map(self):
        ...
        mode = self.combo_map_mode.currentText()

        # 선택된 맵 모드에 따라 맵핑 데이터 스위칭
        if "Thickness" in mode:
            self.map_view.setTitle("Epi Thickness Map (µm)")
            val_key = 't'
        elif "Surface Peak" in mode:
            self.map_view.setTitle("Surface Peak Map (cm⁻¹)")
            val_key = 'surf_peak'
        elif "Substrate Peak" in mode:
            self.map_view.setTitle("Substrate (Intf+10µm) Peak Map (cm⁻¹)")
            val_key = 'peak_10um'
        elif "Difference" in mode:
            self.map_view.setTitle("Peak Difference (Surface - Substrate) (cm⁻¹)")
            val_key = 'peak_diff'

        for (x, y), val in self.data_map.items():
            grid[self.ys.index(y), self.raw_xs.index(x)] = val.get(val_key, 0)
        ...
        if 0 <= ix < len(self.display_xs) and 0 <= iy < len(self.ys):
            ...
            try:
                ...
                if data['t'] > 0:
                    ...
                    # 🚨 [신규] 기판(Intf+10um) 수직선 추가 (초록 점선)
                    if not np.isnan(data.get('z_10um', np.nan)):
                        self.prof_view.addItem(pg.InfiniteLine(data['z_10um'], angle=90,
                                                               pen=pg.mkPen('g', width=2, style=Qt.PenStyle.DotLine)))

            except Exception as e:
                print(f"Plotting Error: {e}")

            # 🚨 [신규] 텍스트 정보에 모든 피크 정보 포맷팅
            info_text = (f"📍 X: {x_val}µm, Y: {y_val}µm\n"
                         f"🎯 Thickness: {data['t']:.3f} µm\n"
                         f"✨ Surf Peak: {data.get('surf_peak', np.nan):.2f} cm⁻¹\n"
                         f"✨ Sub Peak (Intf+10µm): {data.get('peak_10um', np.nan):.2f} cm⁻¹\n"
                         f"⚖️ Diff (Surf - Sub): {data.get('peak_diff', np.nan):.2f} cm⁻¹")
    ...
    def save_map_csv(self):
        ...
        path, _ = QFileDialog.getSaveFileName(self, "Save Map CSV", "epi_analysis_map_full.csv", "CSV Files (*.csv)")
        if not path:
            return
        ...
        for (x, y), val in self.data_map.items():
            export_data.append({
                'X (um)': x,
                'Y (um)': y,
                'Thickness (um)': val.get('t', 0),
                'Surface Z (um)': val.get('zs', 0),
                'Interface Z (um)': val.get('zi', 0),
                'Surface Peak Pos (cm-1)': val.get('surf_peak', np.nan),
                'Substrate +10um Z Depth (um)': val.get('z_10um', np.nan),  # 🚨 10um Z 깊이 저장
                'Substrate +10um Peak Pos (cm-1)': val.get('peak_10um', np.nan),  # 🚨 10um 피크 저장
                'Peak Difference (cm-1)': val.get('peak_diff', np.nan)  # 🚨 피크 차이 저장
            })
